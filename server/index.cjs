...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === '/api/proxy-type' && req.method === 'POST') {
    ...
    try {
      ...
      execSync('xclip -selection clipboard -i <<< ' + JSON.stringify(text), { timeout: 5000, stdio: 'ignore' });
      execSync('DISPLAY=' + JSON.stringify(target) + ' xdotool key ctrl+v', { timeout: 5000, stdio: 'ignore' });
      execSync('DISPLAY=' + JSON.stringify(target) + ' xdotool key Return', { timeout: 5000, stdio: 'ignore' });
  ...
  if (urlPath === '/api/type' && req.method === 'POST') {
    ...
    try {
      ...
      let displayOk = false;
      try {
        execSync('DISPLAY=' + JSON.stringify(target) + ' xdotool getactivewindow', { timeout: 3000, stdio: 'ignore' });
        displayOk = true;
      } catch (e) {
        console.log('[type] Display not accessible, trying via host.docker.internal');
      }
      ...
      if (displayOk) {
        ...
        // 通过 HTTP 代理到主机执行
        const host = 'http://host.docker.internal:13446';
        const proxyRes = await fetch(host + '/api/proxy-type', {
          method: 'POST',
  ...
  if (urlPath === '/api/key' && req.method === 'POST') {
    ...
    try {
      ...
      if (!key || !target) return json(res, { success: false, error: 'need key and target' });
      execSync('DISPLAY=' + JSON.stringify(target) + ' xdotool key -- ' + JSON.stringify(key), { timeout: 5000, stdio: 'ignore' });
      return json(res, { success: true });
    } catch (e) { return json(res, { success: false, error: e.message }); }
  }
